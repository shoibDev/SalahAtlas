...
public class AccountMapperImpl implements Mapper<AccountEntity, AccountDto> {
    ...
    public void init() {
        if(modelMapper.getTypeMap(AccountEntity.class, AccountDto.class) == null){
            modelMapper.typeMap(AccountEntity.class, AccountDto.class)
                    .addMappings(mapper -> {
                       mapper.map(
                               src -> Optional.ofNullable(src.getOrganizedJummah())
                                       .map(JummahEntity::getId)
                                       .orElse(null),
                               AccountDto::setOrganizedJummahId
                       );
                       mapper.map(
    ...
    public AccountEntity mapFrom(AccountDto accountDto) {
        ...
        if(accountDto.getOrganizedJummahId() != null){
           jummahRepository.findById(accountDto.getOrganizedJummahId())
                   .ifPresent(accountEntity::setOrganizedJummah);
        }
